...
const runModes = [
  ...
  { key: 'full', label: 'Full Karnataka Run', budget: 4000, note: 'Main deep discovery run. It may stop before the cap if results dry up.' },
  { key: 'extended', label: 'Extended Run', budget: 5500, note: 'Uses almost all remaining Serper balance. Confirm before running.' },
  { key: 'custom', label: 'Custom', budget: 200, note: 'Set your own query budget.' },
];

const pathwayOptions = [
  { key: 'residential_life_system', label: 'Residential / life-system', note: 'Residential schools, hostels, CCIs with education, children’s homes, tribal residential schools.' },
  { key: 'full_day_alternative', label: 'Full-day / alternative education', note: 'Free schools, alternative schools, dropout/open-school pathways, whole-child schools.' },
  { key: 'child_protection_rehab', label: 'Child protection / rehabilitation', note: 'Child labour rehab, shelters, rescued children, girls’ homes, CWC-linked institutions.' },
  { key: 'disability_special_needs', label: 'Disability / special needs', note: 'Blind schools, hearing-impaired schools, autism/special schools, disability life-skills.' },
  { key: 'sports_arts_stem_vocational', label: 'Sports / arts / STEM / vocational', note: 'Only serious cohort pathways, not camps, workshops, hobby clubs or competitions.' },
  { key: 'exceptional_community_pathway', label: 'Exceptional community pathway', note: 'Off by default. Only rare Katha/Parikrma-like pathways, not generic tuition.' },
];
...
const rowStatus = (r: AnyRow) => field(r, 'Status', 'Website Status', 'Repository Status', 'status');
...
function HowItWorks({ open, onToggle }: { open: boolean; onToggle: () => void }) {
  return <div className="form-card" style={{marginTop: 0}}>
    ...
    <p className="muted-empty">The system searches the web for child institutions where food could genuinely matter. It removes obvious non-fits like volunteer groups, camps, scholarships, awareness programs, adult livelihood work and generic tuition. Then it shows a clean list with source links so the team can manually decide.</p>
    ...
    {open && <div className="advanced-panel" style={{margin:'14px 0 0'}}>
      <p className="muted-empty">It looks separately for residential schools, hostels, children’s homes, full-day schools, child-protection homes, special schools, and serious sports/arts/STEM pathways. It does not try to score NGOs. It only asks: does this organisation hold a child’s pathway long enough for food support to help?</p>
    </div>}
  </div>
}
...
export default function NgoDiscoveryPage(){
  ...
  async function loadDiscoveryArchive(){ if(!BACKEND)return; const r=await safeJSON(`${BACKEND}/discovery/archive?limit=80`); if(r.ok&&r.data)setDiscArchive(r.data.rows||[]); }
  useEffect(()=>{loadDiscoveryArchive();},[]);
  ...
  return <><Header active="repository"/><main className="dfp-wrap page-stack">
    <section className="module-hero"><div className="red-kicker">NGO Discovery Module</div><h1>Find Child <span>Pathways</span></h1><p>Find and verify child trajectory-shifting institutions where food can strengthen attendance, retention, health, safety, rehabilitation or performance.</p><div className="hero-dots"/></section>
    <section className="discover-card">
      ...
      {tab==='general'&&<>
        ...
        <div className="budget-strip"><b>{Number(budget||0).toLocaleString()} / {MAX_DISCOVERY_BUDGET.toLocaleString()} query cap</b><span>{runModes.find(m=>m.key===runMode)?.note || 'Manual query budget'} · The run does not score NGOs; it surfaces a clean list for manual sorting.</span></div>
    ...
    <section className="collapse-card"><button className="collapse-head" onClick={()=>{setDiscArchiveOpen(!discArchiveOpen); loadDiscoveryArchive();}}><span className="mini-icon">▣</span><span><b>Discovery history & reports</b><small>Download older General Discovery runs and audits.</small></span><em>{discArchive.length || 0} runs</em></button>{discArchiveOpen&&<div className="collapse-body"><div className="archive-list">{discArchive.length===0&&<div className="muted-empty">No General Discovery runs found yet.</div>}{discArchive.slice(0,40).map((r,i)=>{const id=String(r.run_id||''); const dl=r.downloads||{}; return <div className="archive-row" key={id||i}><div><b>General Discovery — {r.state||'Statewide'}</b><small>{r.updated_at||'—'} · {r.run_mode||'run'} · {r.processed||0}/{r.total||0} queries · surfaced {r.stories_found||0}</small></div><div className="archive-links">{dl.stories&&<a href={discoveryDownload(id,'leads')}>Clean output</a>}{dl.audit&&<a href={discoveryDownload(id,'audit')}>Audit</a>}{dl.rejected&&<a href={discoveryDownload(id,'rejected')}>Rejected</a>}{dl.errors&&<a href={discoveryDownload(id,'errors')}>Errors</a>}{(r.stage==='paused'||r.run_status==='paused')&&<button onClick={()=>resumeDiscovery(id)}>Resume</button>}</div></div>})}</div></div>}</section>
    {!discStatus&&!repoStatus&&!discResults&&!repoResults&&<div className="empty-dark">Start with Test Run — 200 queries. Review output, then scale to Standard, Full Karnataka, or Extended Run.</div>}
